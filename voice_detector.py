...
def convert_voice_advanced(input_path, model, original_voice_features):
    ...
    if matching_original and os.path.exists(matching_original):
        ...
        return y_orig, output_path, closest_speaker
    ...
    for orig_features, orig_path in original_voice_features:
        ...
        if distance < min_distance:
            ...
            closest_speaker = os.path.basename(orig_path).split('_')[0]  # Extract speaker name
    ...
    return y_converted, output_path, closest_speaker
...
def upload_file():
    ...
    try:
        ...
        if not file_path.lower().endswith('.wav'):
            wav_file_path = os.path.join(uploads_dir, os.path.splitext(filename)[0] + '.wav')
            convert_to_wav(file_path, wav_file_path)
        else:
            wav_file_path = file_path
        
        # Process the audio file
        result = process_audio(wav_file_path)
        
        # Unpack the result
        output_path, extracted_features, authenticity, gender, emotion = result
        
        # Format the output message
        output_message = f"Audio is {authenticity} and gender is {gender}. Emotional tone: {emotion}."
        ...
        response_data = {
            ...
            'gender': gender,
            'emotion': emotion,
            'features': extracted_features.tolist() if isinstance(extracted_features, np.ndarray) else extracted_features,
            'download_url': f"/download/{os.path.basename(output_path)}" if output_path else f"/download/{os.path.basename(wav_file_path)}",
            'original_url': f"/download/{os.path.basename(wav_file_path)}",
            'full_path': output_path if output_path else wav_file_path
        }
        
        # Debug log
        print("Sending response with file paths:")
        print(f"Original file: {wav_file_path}")
...
def download_file(filename):
    """Serve the audio file for download or playback"""
    try:
        ...
        # Check in static/uploads directory next
        static_uploads_dir = os.path.join(os.getcwd(), 'static', 'uploads')
        if os.path.exists(os.path.join(static_uploads_dir, filename)):
            return send_from_directory(static_uploads_dir, filename)
        
        # If not found in standard locations, search for it
        for root, dirs, files in os.walk(os.getcwd()):
            if filename in files:
                directory, file = os.path.split(os.path.join(root, filename))
...
def process_audio(file_path):
    ...
    # Extract features - use the correct parameter name
    extracted_features = extract_features(audio_path=file_path)
    
    # If the voice is fake, convert it
    if authenticity == 'fake':
        # Convert the fake voice to original
        converted_audio, output_path, closest_speaker = convert_voice_advanced(file_path, model, original_voice_features)
    else:
        output_path = None
    
    print(f"Processed audio. Original: {file_path}, Converted: {output_path}")
...
if __name__ == '__main__':
    ...
    auth_classifier, gender_classifier, auth_metrics, gender_metrics = train_models(data, labels_authenticity, labels_gender)
    
    # Initialize the voice conversion model
    model = AdvancedVoiceConverter(input_dim=5)
    
    # Skip the visualization functions that are causing errors
